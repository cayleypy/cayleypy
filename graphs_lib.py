...
class PermutationGroups:
    ...
    @staticmethod
    def lsl(n: int) -> CayleyGraphDef:
        """Cayley graph for S_n (n>=3), generated by: 
        - L = left shift (longest cycle)
        - S = sub-longest cycle (fixes 0, cycles 1..n-1 in same direction as L).

        :param n: Size of permutations (n >= 3).
        """
        assert n >= 3

        # Longest cycle (left shift): [1, 2, ..., n-1, 0]
        L = list(range(1, n)) + [0]

        # Sub-longest cycle: 0 fixed, [1 -> 2 -> ... -> n-1 -> 1]
        S = [0] + list(range(2, n)) + [1]
...
def prepare_graph(name: str, n: int = 0, **kwargs) -> CayleyGraphDef:
    ...
    if name == "cube_2/2/2_6gensQTM":
        ...
        return PermutationGroups.lsl(n)
