...
def bfs_numpy(self, max_diameter: int = 1000000) -> list[int]:
    ...
    perm_funcs = [
        self.string_encoder.implement_permutation_1d(p) for p in perms]
    pn = len(perms)
    start_state = self._encode_states(
        self.destination_state).cpu().numpy().reshape(-1)
