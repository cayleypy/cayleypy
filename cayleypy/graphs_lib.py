...
def prepare_graph(name: str, n: int = 0, **kwargs) -> CayleyGraphDef:
    ...
    Supported graphs:
      ...
      * "heisenberg" - Cayley graph for group of upper triangular 3x3 integer matrices with 1s on main diagonal.
          ...
          Has parameter p - multiplication modulo. 0 by default (meaning no modulo).
    ...
    if name == "all_transpositions":
        ...
        p = kwargs.get("p", 0)
        x = MatrixGenerator.create([[1, 1, 0], [0, 1, 0], [0, 0, 1]], modulo=p)
        x_inv = MatrixGenerator.create([[1, -1, 0], [0, 1, 0], [0, 0, 1]], modulo=p)
        y = MatrixGenerator.create([[1, 0, 0], [0, 1, 1], [0, 0, 1]], modulo=p)
        y_inv = MatrixGenerator.create([[1, 0, 0], [0, 1, -1], [0, 0, 1]], modulo=p)
