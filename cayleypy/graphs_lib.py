...
class PermutationGroups:
    ...
    @staticmethod
    def rmk(n, m=0, k=3) -> CayleyGraphDef:
        """ Cayley graph for S_n (n>=3),

        :param n: Size of permutations.
        :param m, k: Define second generator (M, M + 1, ..., M + K − 1)
        :return:
        """
        assert n > 3
        generators = [[i for i in range(1, n+1)] + [0], [i for i in range(m)] + [m+k-1] + [i for i in range(m, m + k - 1)] + [i for i in range(m + k, n + 1)]]
        generator_names = ["R", "C"]
        name = f"rmk-{n}"
        return CayleyGraphDef.create(
            generators,
