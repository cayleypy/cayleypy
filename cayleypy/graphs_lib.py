...
class PermutationGroups:
    ...
    def all_cycles(n: int) -> CayleyGraphDef:
        """Cayley graph for S_n (n>=2), generated by all possible cycles of length >= 2."""
        assert n >= 2
        generators = []
        generator_names = []
        
        for k in range(2, n+1):  # cycle lengths from 2 to n
            for subset in combinations(range(n), k):  # all subsets of length k
                min_elem = min(subset)  # minimal element A
                rest = [x for x in subset if x != min_elem]

                for perm in permutations(rest):  # all perms of rest
                    # construct the cycle as a permutation of length n
                    cycle = list(range(n))  # identity permutation
                    current = min_elem
                    for target in perm:
                        cycle[current] = target
                        current = target
                    cycle[current] = min_elem  # close the cycle

                    generators.append(cycle)
                    generator_names.append(f"({' '.join(map(str, subset))})")
