...
from cayleypy.permutation_utils import apply_permutation, compose_permutations
...
def prepare_graph(name, n=0) -> CayleyGraph:
    ...
    Supported graphs:
      ...
      * "three_cycles_0ij" – Cayley graph for Sₙ (n ≥ 3), generated by 3-cycles of the form (0 i j), where i != j.
    ...
    if name == "all_transpositions":
        ...
        for a, b, c in permutations(range(n), 3):
            if a < b and a < c:
                vector = list(range(n))
                vector[a], vector[b], vector[c] = b, c, a
        ...
        for i in range(1, n):
            for j in range(1, n):
                if j == i:
                    continue
                vector = list(range(n))
                vector[0], vector[i], vector[j] = i, j, 0
                generators.append(vector)
