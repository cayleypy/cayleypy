...
def prepare_graph(name, n=0, **kwargs) -> CayleyGraph:
    ...
    Supported graphs:
      ...
      * "cubic_pancake" - Cayley graph for S_n (n>=2), generated by set of 3 prefix reversal generators.
        Sets definitions are:
          subset=1 => {R_n, R_n-1, R_2}
          subset=2 => {R_n, R_n-1, R_3}
          subset=3 => {R_n, R_n-1, R_n-2}
          subset=4 => {R_n, R_n-1, R_n-3}
          subset=5 => {R_n, R_n-2, R_2}
          subset=6 => {R_n, R_n-2, R_3}
          subset=7 => {R_n, R_n-2, R_n-3}
