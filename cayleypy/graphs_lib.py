...
class PermutationGroups:
    ...
    def increasing_k_cycles(n: int, k: int) -> CayleyGraphDef:
        ...
        Notes:
        - If k is even: each k-cycle is an odd permutation; the graph should cover S_n.
        - If k is odd: each k-cycle is an even permutation; the graph lies in A_n.

        Parameters
        ----------
        n : int
            Size of the symmetric group S_n.
        k : int
            Cycle length for generators.
        """
        ...
        for combo in combinations(range(n), k):
            ...
            if k >= 3:
                inv = [cyc[0]] + list(reversed(cyc[1:]))
                generators.append(permutation_from_cycles(n, [inv]))
                generator_names.append(f"({','.join(map(str, inv))})")
