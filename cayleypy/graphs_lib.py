...
def prepare_graph(name, n=0, **kwargs) -> CayleyGraph:
    ...
    Supported graphs:
      ...
      * "cubic_pancake" - Cayley graph for S_n (n>=2), generated by set of 3 prefix reversal generators. 
        Sets definitions are:
          S=1 => {R_n, R_n-1, R_2},    S=2 => {R_n, R_n-1, R_3},    S=3 => {R_n, R_n-1, R_n-2},
          S=4 => {R_n, R_n-1, R_n-3},  S=5 => {R_n, R_n-2, R_2},    S=6 => {R_n, R_n-2, R_3},
          S=7 => {R_n, R_n-2, R_n-3},
    ...
    if name == "all_transpositions":
        ...
        assert n >= 2
        subset = kwargs['S']
        assert subset in [1, 2, 3, 4, 5, 6, 7]

        def PuncakeGenerator(k, n):
            return list(range(k-1, -1, -1))+list(range(k, n, 1))
        generators = []
        generator_names = []
        if subset == 1:
            generators = [PuncakeGenerator(n, n), PuncakeGenerator(n-1, n), PuncakeGenerator(2, n)]
            generator_names = [f'R{n}', f'R{n-1}', 'R2']
        elif subset == 2:
            generators = [PuncakeGenerator(n, n), PuncakeGenerator(n-1, n), PuncakeGenerator(3, n)]
            generator_names = [f'R{n}', f'R{n-1}', 'R3']
        elif subset == 3:
            generators = [PuncakeGenerator(n, n), PuncakeGenerator(n-1, n), PuncakeGenerator(n-2, n)]
            generator_names = [f'R{n}', f'R{n-1}', f'R{n-2}']
        elif subset == 4:
            generators = [PuncakeGenerator(n, n), PuncakeGenerator(n-1, n), PuncakeGenerator(n-3, n)]
            generator_names = [f'R{n}', f'R{n-1}', f'R{n-3}']
        elif subset == 5:
            generators = [PuncakeGenerator(n, n), PuncakeGenerator(n-2, n), PuncakeGenerator(2, n)]
            generator_names = [f'R{n}', f'R{n-2}', 'R2']
        elif subset == 6:
            generators = [PuncakeGenerator(n, n), PuncakeGenerator(n-2, n), PuncakeGenerator(3, n)]
            generator_names = [f'R{n}', f'R{n-2}', 'R3']
        elif subset == 7:
            generators = [PuncakeGenerator(n, n), PuncakeGenerator(n-2, n), PuncakeGenerator(n-3, n)]
