...
from .moves import MINI_PYRAMORPHIX_ALLOWED_MOVES, PYRAMINX_MOVES, STARMINX_MOVES, STARMINX_2_MOVES, MEGAMINX_MOVES
...
class Puzzles:
    ...
    def starminx_2() -> CayleyGraphDef:
        """Cayley graph associated with the Starminx 2 puzzle is generated by 12 moves, corresponding to rotations of
        the whole dodecahedron’s faces (analogous to face turns in the Megaminx). Unlike the Starminx puzzle, where
        the generators are vertex-centered, the generators in Starminx 2 are face-centered and affect larger regions of
        the puzzle.
        """
        generator_names = list(STARMINX_2_MOVES.keys())
        generators = [STARMINX_2_MOVES[k] for k in generator_names]
        inversed_names = [f"inv_{k}" for k in generator_names]
        generator_names.extend(inversed_names)
        inversed = [inverse_permutation(p) for p in generators]
        generators.extend(inversed)
