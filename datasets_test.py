...
@pytest.mark.skipif(not RUN_SLOW_TESTS, reason="slow test")
def test_lsl_coset_growth():
    for central_state, layer_sizes in load_dataset("lsl_cayley_growth").items():
        n = len(central_state)
        # Проверяем, что общее количество вершин = n!
        assert sum(layer_sizes) == math.factorial(n)
        graph = PermutationGroups.lsl(n).with_central_state(central_state)
        _verify_layers_fast(graph, layer_sizes, max_layer_size=100)
